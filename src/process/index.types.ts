...
export const spawnInput = z.object({
  command: z
    .array(z.string())
    .min(1)
    .describe("Command to run as array of strings: [executable, arg1, arg2, ...]. First element is the program name"),
  options: z
    .object({
      cwd: z.string().optional().describe("Working directory for the spawned process. Defaults to current directory"),
      env: z.record(z.string()).optional().describe("Environment variables to pass to process as key-value pairs"),
      stdin: z
        .enum(["pipe", "inherit", "ignore"])
        .optional()
        .describe("stdin handling: 'pipe' to capture, 'inherit' to use parent stdio, 'ignore' to ignore"),
      stdout: z
        .enum(["pipe", "inherit", "ignore"])
        .optional()
        .describe("stdout handling: 'pipe' to capture, 'inherit' to use parent stdio, 'ignore' to discard"),
      stderr: z
        .enum(["pipe", "inherit", "ignore"])
        .optional()
        .describe("stderr handling: 'pipe' to capture, 'inherit' to use parent stdio, 'ignore' to discard"),
    })
    .optional()
...
export const execInput = z.object({
  ...
  options: z
    .object({
      ...
      env: z.record(z.string()).optional().describe("Environment variables to pass to the command as key-value pairs"),
      shell: z.boolean().optional().describe("If true, run command with shell. Usually handled automatically"),
    })
    .optional()
...
export const processOutput = z.object({
  ...
  success: z.boolean().describe("true if exitCode was 0 (successful), false if exitCode was non-zero (failed)"),
});
